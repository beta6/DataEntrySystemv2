...
(function (factory) {
  ...
  if (!window.jQuery) {
    throw 'Global jQuery not defined';
  }
  ...
  Guillotine = (function () {
    ...
    Guillotine.prototype._wrap = function (element) {
      ...
      this.initialWidth = this.width = width / this.op.width;
      this.initialHeight = this.height = height / this.op.height;
    ...
    Guillotine.prototype._setScale = function (scale) {
      var left, top;
      this.width = this.initialWidth * scale;
      this.height = this.initialHeight * scale;
      this.canvas.style.width = (this.width * 100).toFixed(2) + '%';
      this.canvas.style.height = (this.height * 100).toFixed(2) + '%';
      left = (this.left + 0.5) * (scale / this.data.scale) - 0.5;
      top = (this.top + 0.5) * (scale / this.data.scale) - 0.5;
      this.data.scale = scale;
      return this._offset(left, top);
    };

    Guillotine.prototype._setHeight = function (height) {
      this.height *= (this.op.height / height);
      this.top *= (this.op.height / height);

      this.op.height = this.data.h = height;
      this.initialHeight = this.naturalHeight / this.op.height;

      var paddingTop = this.op.height / this.op.width * 100 + '%';
      this.$gllt.css('padding-top', paddingTop);

      this.$canvas.css({ height: this.height * 100 + '%' });
      return this._offset(this.left, this.top);
    };

    Guillotine.prototype._setWidth = function (width) {
      this.width *= (this.op.width / width);
      this.left *= (this.op.width / width);

      this.op.width = this.data.w = width;
      this.initialWidth = this.naturalWidth / this.op.width;

      var paddingTop = this.op.height / this.op.width * 100 + '%';
      this.$gllt.css('padding-top', paddingTop);

      this.$canvas.css({ width: this.width * 100 + '%' });
      return this._offset(this.left, this.top);
    };
    ...
    Guillotine.prototype._reset = function () {
      if (this.initialWidth >= 1 && this.initialHeight >= 1) {
        this._fit();
      } else {
        this._setScale(1);
      }
    ...
    Guillotine.prototype.rotateLeft = function () {
      return (this._rotate(-90), this._trigger('rotateLeft'));
    };

    Guillotine.prototype.rotateRight = function () {
      return (this._rotate(90), this._trigger('rotateRight'));
    };

    Guillotine.prototype.center = function () {
      return (this._center(), this._trigger('center'));
    };

    Guillotine.prototype.fit = function () {
      return (this._fit(), this._center(), this._trigger('fit'));
    };

    Guillotine.prototype.zoomIn = function () {
      return (this._zoom(this.zoomInFactor), this._trigger('zoomIn'));
    };

    Guillotine.prototype.zoomOut = function () {
      return (this._zoom(this.zoomOutFactor), this._trigger('zoomOut'));
    };

    Guillotine.prototype.setScale = function (scale) {
      return (this._setScale(parseFloat(scale)), this._trigger('setScale'));
    };

    Guillotine.prototype.setHeight = function (height) {
      return (this._setHeight(parseFloat(height)), this._trigger('setHeight'));
    };

    Guillotine.prototype.setWidth = function (width) {
      return (this._setWidth(parseFloat(width)), this._trigger('setWidth'));
    };

    Guillotine.prototype.reset = function () {
      return (this._reset(), this._trigger('reset'));
    };
    ...
    Guillotine.prototype.enable = function () {
      this.$gllt.removeClass('disabled');
      return this.enabled = true;
    };

    Guillotine.prototype.disable = function () {
      this.$gllt.addClass('disabled');
  ...
  whitelist = ['rotateLeft', 'rotateRight', 'center', 'fit', 'zoomIn', 'zoomOut', 'setScale',
    'setHeight', 'setWidth', 'reset', 'instance', 'getData', 'enable', 'disable', 'remove'];
  ...
  $.fn[pluginName] = function (options) {
    ...
    if (typeof options !== 'string') {
      ...
      return this.each(function () {
        ...
        if (guillotine) {
          return guillotine[options].apply(guillotine, _args);
        }
      });
    }
  };
